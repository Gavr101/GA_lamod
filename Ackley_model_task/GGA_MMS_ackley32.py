...
class Individ_GGA_MMS_M(Individ_GGA_MMS_M):
    """Male individ"""
    ...
    def _get_step_chrom(range_chrom_float, bit_chrom):
        ...
        return Individ_GGA_MMS_M._get_step_chrom(range_chrom_float, bit_chrom)
...
class Individ_GGA_MMS_F(Individ_GGA_MMS_F):
    """Female individ."""
    range_chrom_float = RANGE_Chrom_Float
    bit_chrom = BIT_Chrom
    ...
    def _get_step_chrom(range_chrom_float, bit_chrom):
        """Supportive function to determine the step_chrom."""
        return Individ_GGA_MMS_F._get_step_chrom(range_chrom_float, bit_chrom)
...
class Population_GGA_MMS_M(Population_GGA_MMS_M):
    class_Ind = Individ_GGA_MMS_M


class Population_GGA_MMS_F(Population_GGA_MMS_F):
...
for n in range(1, 101):
    DIR = f"Exp__GGA_MMS_correct_criter/{dim}_dim/ackley"
